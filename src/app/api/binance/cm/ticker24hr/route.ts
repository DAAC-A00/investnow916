...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Binance COIN-M API 오류: ${response.status} ${response.statusText}`);
    }
    ...
    console.error('Binance COIN-M 24hr ticker 프록시 오류:', error);
    return NextResponse.json(
      { error: 'Binance COIN-M 24hr ticker 데이터를 가져오는데 실패했습니다.' },
      { status: 500 }
    );
  }
} 